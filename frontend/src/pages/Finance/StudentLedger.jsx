...
const StudentLedger = () => {
  ...
  const { data, isLoading: loading, refetch } = useGetAccountLedgerQuery(studentId);
  ...
  const [modal, setModal] = useState(null); // 'payment' | 'charge'
  const [formData, setFormData] = useState({
    amount: '',
    debitAccountId: '',
    creditAccountId: '',
    description: '',
    category: 'Membership Income', // Used for Credit Account label in charge
  ...
  const handleModalClose = () => {
    setModal(null);
    setFormData({ amount: '', debitAccountId: '', creditAccountId: '', description: '', category: 'Membership Income', date: new Date().toISOString().split('T')[0] });
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm no-print">
        ...
        <div className="flex flex-wrap gap-2">
            ...
            <button onClick={() => setModal('credit')} className="bg-sky-50 text-sky-700 px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-sky-100 transition-all border border-sky-100">
              Credit Note
            </button>
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
        <div className="px-10 py-8 border-b border-slate-50 flex items-center justify-between no-print">
          ...
          <div className="flex items-center gap-3">
            <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Sort: Latest First</span>
          </div>
        </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {loading && transactions.length === 0 ? (
                ...
                return (
                  <tr key={tx._id} className="group hover:bg-slate-50/40 transition-all">
              ...
              {transactions.length === 0 && !loading && (
                <tr><td colSpan="6" className="p-32 text-center text-slate-300 font-bold italic">This account has no transaction history yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
            ...
            >
              ...
              <div className="flex items-center gap-5 mb-10">
                 <div className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center bg-slate-50 ${
                    modal === 'payment' ? 'text-emerald-500' : 'text-rose-500'
                 }`}>
                    {modal === 'payment' ? <TrendingUp size={32} /> : <TrendingDown size={32} />}
                 </div>
                 <div>
                    <h2 className="text-3xl font-black text-slate-900 tracking-tight">
                        {modal === 'payment' ? 'Receive Fee Payment' : 'Add New Charge'}
                    </h2>
                  ...
                  {modal === 'payment' ? (
                    <div className="col-span-1">
                      ...
                      >
                        ...
                        {accounts.filter(a => a.type === 'Assets').map(a => <option key={a._id} value={a._id}>{a.name}</option>)}
                      </select>
                    </div>
                  ) : (
                    <div className="col-span-1">
                ...
                <div className="pt-6">
                  ...
                  >
                    {submitting ? <Loader2 className="animate-spin" size={24} /> : (
                      <>
                        <PlusCircle size={20} />
                        {modal === 'payment' ? 'Complete Receipt' : 'Record Charge'}
                      </>
                    )}
                  </button>
                </div>
