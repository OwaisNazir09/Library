...
const AdminDashboard = () => {
  const { data: adminData, isLoading: loading, error, refetch } = useGetAdminDashboardQuery();
  ...
  if (error) return (
    <div className="flex flex-col items-center justify-center h-full gap-4">
      <p className="text-slate-500 font-bold">{error.data?.message || 'Failed to load system metrics'}</p>
      <button onClick={() => refetch()} className="bg-[#044343] text-white px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest">Retry</button>
    </div>
  );
